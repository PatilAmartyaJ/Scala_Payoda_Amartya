    ...
    Behaviors.receive { (ctx, msg) =>
      msg match {
        ...
        case ProcessBookingEvent(eventJson) =>
          ...
          val eventType= root.fields("type").convertTo[String]
          ...
          if(eventType == "BOOKING_CREATED") {
            ...
            roomService ! RoomServiceActor.SendWelcomeEmail(email,fullName,roomNumber, roomTypeName)
            wifiService ! WifiServiceActor.SendWifiCredentials(email, fullName, roomNumber, wifiUser, wifiPass)
            restaurantService ! RestaurantServiceActor.StartMenuService(email, roomNumber,roomNumber)
          }
          else{
            roomService ! RoomServiceActor.SendThankYouEmail(email, fullName)
          }
          Behaviors.same
